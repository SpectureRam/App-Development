...
public class UserController {
    ...
    public ResponseEntity<String> addToCart(@RequestBody Map<String, Long> request) {
        Long uid = request.get("userId");
        Long productId = request.get("productId");

        try {
            userService.addToCart(uid, productId);
    ...
    public ResponseEntity<List<CartItemDTO>> getUserCart(@PathVariable Long userId) {
        List<CartItemDTO> cartItems = userService.getUserCartWithDetails(userId);
    ...
    @PutMapping("/{userId}/cart/{itemId}/decrease")
    ...
    @PutMapping("/{userId}/cart/{itemId}/increase")
